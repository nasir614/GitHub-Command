...
class _EditableBulletListState extends State<EditableBulletList> {
  ...
  void initState() {
    ...
    _bullets = BuiltList<Tuple2<BulletEntity, FocusNode>>(
        widget.initialBulletPoints.map(
  ...
  Widget build(BuildContext context) {
    ...
    children = _bullets
        .sublist(0, end > 0 ? end : 0)
        .map((bullet) => _buildRow(bullet: bullet))
        .toList();
    ...
    try {
      ...
      if (children.isEmpty) {
        _bullets = _bullets.rebuild((b) => b
          ..add(Tuple2(BulletEntity(text: ' ', checked: false), FocusNode())));
        children.add(_buildRow(bullet: _bullets.last));
      }
    }
        ...
        onChanged: (value) {
          ...
          if (id >= 0) {
            setState(() {
              ...
              b[id] = Tuple2(
                  BulletEntity(text: bullet.item1.text, checked: value),
                  bullet.item2);
              _bullets = b.build();
            });
          }
            ...
            if (id >= 0) {
              if (value.isEmpty) {
                ...
                setState(() {
                  final b = _bullets.toBuilder();
                  b[id] = Tuple2(
                      BulletEntity(text: value, checked: bullet.item1.checked),
                      bullet.item2);
                  _bullets = b.build();
                });
              }
