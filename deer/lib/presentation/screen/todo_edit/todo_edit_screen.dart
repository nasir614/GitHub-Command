...
class _TodoEditScreenState extends State<TodoEditScreen> {
  ...
  void _setupNotification(TodoEditState state) async {
    ...
    final time = await showTimePicker(
      context: context,
      initialTime:
          TimeOfDay.fromDateTime(state.todo.notificationDate ?? DateTime.now()),
    );
    ...
    if (!state.todoNameHasError) {
      if (state.todo.notificationDate == null) {
        cancelNotification(state.todo);
          ...
          widget.todo.notificationDate.compareTo(state.todo.notificationDate) !=
              0) {
    ...
    return ShadedBox(
      child: Padding(
        ...
        child: _TextField(
          ...
          fontSize: 20.0,
          hint:
              state.todoNameHasError ? 'Name can\'t be empty' : 'Todo\'s name',
          onChanged: (value) =>
