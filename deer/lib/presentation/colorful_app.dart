...
class ColorfulApp extends StatefulWidget {
  ...
  static ColorfulAppState of(BuildContext context) {
    return context.findAncestorStateOfType<ColorfulAppState>();
  }
...
class ColorfulAppState extends State<ColorfulApp> {
  ...
  void _loadColorTheme() async {
    final prefs = await SharedPreferences.getInstance();
    final theme = stringToEnum(
            prefs.getString(_colorfulThemeKey), ColorfulTheme.values) ??
        ColorfulTheme.standard;
...
class ColorThemeData {
  ...
  ThemeData get theme => ThemeData(
        ...
        textTheme: TextTheme(
          subtitle1: TextStyle().copyWith(fontSize: 14.0),
        ),
        colorScheme: ColorScheme.fromSwatch().copyWith(secondary: _medium),
        textSelectionTheme: TextSelectionThemeData(
          cursorColor: _bleak,
          selectionColor: _bright,
          selectionHandleColor: _medium,
        ),
      );
